...
# Sources and labels are documented in Oracle_lab_10_proforma.md.
ASSUMPTIONS = {
    ...
    # OCI data-center capacity spending is the company-specific line.
    "capex_to_revenue": [0.70, 0.55, 0.40, 0.30, 0.25],  # judgment
    "tax_rate": 0.199,  # judgment; FY2026 adjusted rate disclosed in the 10-K
...
def assert_balanced(year: int, row: dict[str, float]) -> None:
    """Stop the valuation if cash is typed or the balance sheet is wrong."""
...
def project() -> list[dict[str, float]]:
    ...
    for index, year in enumerate(YEARS):
        ...
        # Oracle has no separately disclosed, material inventory balance.  The cash
        # flow therefore uses the change in other operating assets instead.
...
def value_equity(rows: list[dict[str, float]]) -> tuple[float, float]:
    """Value positive FCFE; the final forecast FCFE supports the terminal value."""
    a = ASSUMPTIONS
    ...
    terminal_value = (
        rows[-1]["fcfe"] * (1 + a["terminal_growth"])
...
def print_table(title: str, rows: list[dict[str, float]], keys: list[tuple[str, str]]) -> None:
    print(f"\n{title}")
